...
OMY uses Zenoh RMW by default. If you want to use Fast DDS instead, update `RMW_IMPLEMENTATION` in `~/.bashrc` inside every container that runs ROS 2 nodes.(open_manipulator, cyclo intelligence, cyclo manager, etc.)

Connect the ssh to the robot and enter the container.

```bash
ssh robotis@omy-snpr44b0000.local
./data/docker/open_manipulator/docker/container.sh enter
```
